# Este programa trabaja con Noto en vexz de con Twemoji
...
def incluye_emojis():
    global manual_1, restos_noto

    noto_completo = os.listdir(ucdef.DIRECTORIO_NOTO)

    restos_noto = []

    # elimina caracter FE82B que está como emoji (una bandera con interrogante)
    # pero que creo que no existe en unicode
    for i in range(len(noto_completo) - 1, -1, -1):
        if noto_completo[i] == "emoji_ufe82b.svg":
            del noto_completo[i]

    for i in noto_completo:
        i2 = i.replace(".svg", "")
        i2 = i2.replace("emoji_u", "").upper()
        if i2.find("_") == -1:
            i2 = [i2]
        else:
            i2 = i2.split("_")
    ...
    cuenta_notos_asociados = 0
    cuenta_caracteres_no_asociados = 0
    for c in manual_1:
        if c[2] != []:
            cuenta_notos_asociados += 1
        else:
...
def exporta_lista():
    destino = ucdef.FICHERO_MANUAL_1_NOTO
    print()
...
def main():
    print()
    print("93. CREANDO FICHEROS MANUALES A PARTIR FUSIONADOS_2")
    print()
    crea_manual_1()
    incluye_emojis()
    # Comprueba si el fichero de destino existe y pide confirmación para sobreescribirlo
    p_1 = pathlib.Path(ucdef.FICHERO_MANUAL_1_NOTO)
    respuesta = "N"
    if p_1.exists():
